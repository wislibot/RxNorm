-- Force-replace rx_match_medication_lines with fixed token pass (no regex quantifier bug)
...
as $$
    ...
    -- For each input line, find ingredients where ALL tokens appear (exact or stem).
    -- Uses indexed lookup on rx_ingredient_tokens instead of regex scan.
    token_candidates as (
        select
            il.input_index,
            t.ingredient_id
